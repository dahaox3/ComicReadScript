...
脚本可以连接本地运行的 [Reline Local GUI](https://github.com/dahaox3/reline-local-GUI) API 服务，把漫画页面发送到本地 Reline 流水线处理，再把返回结果显示在阅读模式中。

启用后可以在阅读模式设置中配置 Reline 服务地址，默认是：
...
- 对当前页手动执行 Reline 超分，并可再次点击隐藏/显示结果
- 自动预处理当前页、前后若干页或整章内容
- 可选择是否处理当前页之前的页面
- 在滚动条中显示 Reline 等待、处理中、完成和错误状态
- 结果会在当前阅读会话中缓存，重复查看同一页时不需要再次处理
- 缓存数量可以在阅读模式设置中配置，支持 50 / 100 / 200 张或无限制

Reline 侧可以使用 GUI 中的 `API Output` 节点决定返回给 ComicRead 的图片阶段，也可以使用 `API Snapshot` 节点额外保存本地副本。`API Output` 节点可以选择返回格式，默认 `jpeg` 以降低传输体积和缓存占用，也可以切换为 `png`。
